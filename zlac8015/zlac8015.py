...
class Controller:
	...
	def __init__(self, port="/dev/ttyUSB0", ID=1):
		...
		######################
		## Common
		self.CONTROL_REG = 0x2031
		self.OPR_MODE = 0x2032
		self.ACL_TIME = 0x2037
		self.L_DCL_TIME = 0x2038
	...
	def set_accel_time(self, ms):
		...
		result = self.client.write_registers(self.ACL_TIME, int(ms), slave=self.ID)


	def set_decel_time(self, L_ms):
	...
	def set_rpm(self, L_rpm):
		...
		result = self.client.write_registers(self.L_CMD_RPM, left_bytes, slave=self.ID)
	...
	def get_rpm(self):
		...
		registers = self.modbus_fail_read_handler(self.L_FB_RPM, 1)
		fb_L_rpm = np.int16(registers)/10.0
